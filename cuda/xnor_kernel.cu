...
__global__ void xnor_gemm_kernel(uint32_t *A, uint32_t *B, float *C, int m, int n, int k) {
    ...
    //this is the best code that I coded until now
    ...
    // Write Csub to device memory, and each thread writes one element
...
torch::Tensor xnor_gemm_cuda(torch::Tensor &input_a, torch::Tensor &input_b)
{
